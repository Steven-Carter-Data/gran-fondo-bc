...
def main():
    ...
    # Debug buttons for each athlete
    if st.sidebar.button("🔍 Debug Jeremiah's Data"):
        debug_athlete_data("Jeremiah")
    
    if st.sidebar.button("🔍 Debug Steven's Data"):
        debug_athlete_data("Steven")
        
    if st.sidebar.button("🔍 Debug Matt's Data"):
        debug_athlete_data("Matt")

def debug_athlete_data(athlete_name_filter):
    """Debug function to show detailed data for a specific athlete"""
    st.subheader(f"🔍 {athlete_name_filter} Debug Data")
    ...
    if not hr_zones_df.empty:
        # Filter for the specified athlete
        athlete_data = hr_zones_df[hr_zones_df['athlete_name'].str.contains(athlete_name_filter, na=False)]
        
        st.write(f"**{athlete_name_filter}'s Week 1 HR Zone Data:**")
        st.write(f"Total records: {len(athlete_data)}")
        
        if not athlete_data.empty:
            # Show columns available
            st.write("**Available columns:**", list(athlete_data.columns))
            ...
            # Also show what the app's calculation function produces
            st.write("**App's Calculation Function Result:**")
            app_points = calculate_hr_zone_points(athlete_data)
            if not app_points.empty:
                st.dataframe(app_points)
            else:
                st.write("No points calculated by app function")
        else:
            st.write(f"No data found for {athlete_name_filter}")
    else:
        st.write("No HR zone data found for Week 1")

    # Also check if there are activities without HR zone data
    st.write(f"**{athlete_name_filter}'s Activities without HR Zone Data:**")
    activities_df = fetch_activities_by_date_range(supabase, week1_start, week1_end)
    if not activities_df.empty:
        athlete_activities = activities_df[activities_df['athlete_name'].str.contains(athlete_name_filter, na=False)]
        if not athlete_activities.empty:
            st.write(f"Total activities for {athlete_name_filter}: {len(athlete_activities)}")
            st.dataframe(athlete_activities[['activity_name', 'start_date', 'sport_type', 'distance', 'moving_time']])
            
            # Check which activities have HR zone data
            hr_activity_ids = set(athlete_data['activity_id'].tolist()) if not athlete_data.empty else set()
            all_activity_ids = set(athlete_activities['id'].tolist())
            missing_hr_data = all_activity_ids - hr_activity_ids
            
            if missing_hr_data:
                st.warning(f"⚠️ {len(missing_hr_data)} activities missing HR zone data")
                missing_activities = athlete_activities[athlete_activities['id'].isin(missing_hr_data)]
                st.dataframe(missing_activities[['activity_name', 'start_date', 'sport_type']])
            else:
                st.success("✅ All activities have HR zone data")
